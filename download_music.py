...
window = Tk()
weather = StringVar()
window.geometry('800x400+500+200')  # 窗口大小
window.title('网络音乐歌曲免费下载===下载速度取决于您的网速')

#背景图

#bgphoto = PhotoImage(file=r"D:\1.gif")
#label_bg = Label(window, image=bgphoto)
#label_bg.pack()
...
#文本框
taitouLabel = Label(window, text="请输入要下载的歌曲:  ", height=2, width=30, font=("宋体", 20, "bold"), fg="#0489B1")
sousuoLabel = Label(window, text="选择音乐库:  ", height=1, width=15, font=("楷体", 15, "bold"), fg="#04B404")
xialalan = Combobox(window, width=4, textvariable=weather, font=("宋体", 15, "normal"), state='editable')
xialalan['values'] = ('QQ', '酷我', '虾米', '酷狗', '百度', '网易')
xialalan.current(0)

namee_Entry = Entry(window, width=42, font=("宋体", 15, "normal"))

button = Button(window, text="搜索", command=sousuo, )  # .grid_location(33,44)
GunDongTiao = Scrollbar(window)  # 设置滑动块组件
confirmLabel = Listbox(window, height=15, width=55, font=("宋体", 12, "normal"), fg='#40FF00', bg='#FFFFFF',
